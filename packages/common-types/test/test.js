...
describe("Point", function () {
  it("#should encode into elliptic format on encode", async function () {
    ...
    const result = point.encode("elliptic-compressed", { ec: secp256k1 });
    if (result.toString().slice(2) !== point.x.toString("hex", 64)) {
      fail(`elliptic format x should be equal ${secret} ${result.toString()} ${point.x.toString("hex")} ${secret.umod(secp256k1.n)}`);
    }
  });
  ...
  it("#should decode into point for elliptic format compressed", async function () {
    ...
    const key = secp256k1.keyFromPublic(result.toString(), "hex");
